...
export default function Navbar() {
  ...
  return (
    <nav style={{background: 'linear-gradient(to right, #00958f, #00b193)'}} className="text-white px-4 md:px-6 py-4 shadow-xl">
      <div className="flex items-center justify-between">
        {/* Logo */}
        <Link to="/" className="text-xl md:text-2xl font-bold text-white hover:text-yellow-300 transition flex items-center gap-2 flex-shrink-0">
        ...
        <div className="hidden md:flex items-center gap-6">
          {user && (
            <>
              {/* Wishlist Icon */}
              <Link to="/wishlist-page" className="relative hover:bg-white/20 p-2 rounded-lg transition">
      ...
      {user && mobileMenuOpen && (
        <div className="md:hidden mt-4 pb-4 space-y-3 border-t border-white/20 pt-4">
          ...
          <Link 
            ...
            className="flex items-center gap-3 hover:bg-white/20 px-4 py-2 rounded-lg transition w-full"
          >
            <div className="w-10 h-10 rounded-full overflow-hidden bg-white/20 flex items-center justify-center border-2 border-yellow-300 shadow-lg flex-shrink-0">
              {user.profileImage ? (
                <img 
                  src={user.profileImage.startsWith('http') ? user.profileImage : `https://electromart-backend-m2oz.onrender.com${user.profileImage}`} 
                  alt={user.name} 
            ...
            <div className="flex-1">
              ...
              <span className="text-white/80 text-xs">View Profile</span>
            </div>
          </Link>
          ...
          <button
            ...
            className="w-full bg-red-500 hover:bg-red-600 px-4 py-2 rounded-lg font-semibold transition flex items-center justify-center gap-2 shadow-lg"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
            </svg>
